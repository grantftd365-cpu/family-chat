...
- `agent_relationship_profiles` 表：保存“某个 Agent 对某个人”的长期认识，包括熟悉度、亲近感、信任、张力、近期主题和观察到的表达/认知特征。
- `agent_growth_events` 表：保存从聊天中沉淀出的成长证据，用于主动联系、后续记忆整理和产品测试。
...
3. 群里每个非发送者 Agent 先 `observe()`：
   ...
   - 同步更新该 Agent 对当前发言人的关系画像。
4. 被 `@` 或概率命中的 Agent 再 `think()`：
   - 读取当前 session 短期上下文。
   - 读取核心记忆。
   - 读取当前群成员快照，明确谁是真人、谁是数字人。
   - 读取“我对当前发言人的认识”。
...
- 关系画像会区分真人成员和数字人成员，避免把“我”“老婆”“系统”混在一起。

这能支撑“我沉稳有思想”“老婆东北人泼辣”“偶尔吵架但有分寸”的家庭群人格关系演化。

## 主动联系

主动联系不能只是随机问候。当前实现会优先从 `agent_growth_events` 读取最近的真实互动线索，例如某人提到 SAP/APO、供应链、情绪压力或家庭分歧，再生成“像微信里自然开口”的主动话题。

如果没有成长事件，才回退到 `proactive_config.topics` 或默认打招呼。主动消息仍进入同一个 `group:{group_id}` session，并且如果已配置音色，会尝试生成语音消息。
